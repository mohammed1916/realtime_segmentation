...
class CustomDataset_cityscape_clips(Dataset):
    ...
    def remove_tmp_files(self, results):
        """Remove tmp files after evaluation."""
        for result in results:
            if isinstance(result, str):
                if os.path.exists(result):
                    os.remove(result)
            else: 
                raise ValueError("The result is not a file path")
...
class CustomDataset_video2(Dataset):
    ...
    def __init__(self,
        ...
        self.img_all=[]
        remove_vid = []		
        self.dilation_offset = []
        cnt = 1
        self.videolists = sorted(self.videolists)
        
        for video in self.videolists:#[:2]: # Change here for shorter infer
            v_path = os.path.join(self.data_root,'data',video,'origin')
            imglist = sorted(os.listdir(v_path))
            if self.dilation_mode and len(imglist)<self.max_dilation:
                remove_vid.append(video)
                print("skipped:", remove_vid)
                continue

            self.imgdic[video]=imglist
            self.img_all=self.img_all+[[video, img] for img in imglist]
            self.dilation_img_all = self.dilation_img_all + [[video, img] for img in imglist[self.max_dilation-1:]]
            
            self.dilation_offset+= [cnt*(self.max_dilation-1)]*(len(imglist)-self.max_dilation+1)
            cnt+=1
        
        for video in remove_vid:
            self.videolists.remove(video)
    ...
    # Function to check if the video starts/ends with the given index
    def is_video_edge(self, idx, start=True):
        # Unsupported with dilation
        if self.dilation_mode:
            raise NotImplementedError
        
        # Get the parent directory of the video for the frame in the index
        if start:
            if self.img_all[idx][0] == self.img_all[idx-1][0]:
                return False
        else:
            if self.img_all[idx][0] == self.img_all[idx+1][0]:
                return False
    ...
    def __len__(self):
        ...
        # return len(self.img_infos)
        if self.dilation_mode:
            return len(self.img_all) - (self.max_dilation-1) * len(self.videolists)
        elif self.split=='train' or self.split=='train_val_generate_prototype':
            return len(self.videolists)
        else:
            return len(self.img_all)
        # return len(self.img_all)
    ...
    def __getitem__(self, idx):
        ...
        elif self.mamba_mode:
            return self.prepare_test_img3(idx)
        elif self.test_mode:
    ...
    def prepare_test_img2(self, idx):
        ...
        if self.dilation_mode:
            idx = idx + self.dilation_offset[idx]
    ...
    def prepare_test_img3(self, idx):
        """Get testing data after pipeline for video eval with mamba.

        Args:
            idx (int): Index of video data.
    ...
    def format_results(self, results, save_path=None, **kwargs):
        ...
        # pass
        ## changed by guosun
        if not self.dilation_mode:
            assert len(results)==len(self.img_all)
        else:
            print(len(results),len(self.dilation_img_all))
            assert len(results)==len(self.dilation_img_all)
        palette_list=[]
        ...
        for ii in range(len(results)):
            result=results[ii]
            if self.dilation_mode:
                video_imgname=self.dilation_img_all[ii]
            else:
    ...
    def unformat_results(self, save_path=None, **kwargs):
        '''
            This function unformats to create the result file for evaluation
        '''
        ## changed by Ash
        results = []
        
        for ii in range(len(self.img_all)):
            video_imgname=self.img_all[ii]
            video, img_name=video_imgname[0], video_imgname[1]
            save_path_ii=os.path.join(save_path,video,img_name.replace(self.img_suffix, self.seg_map_suffix))
            # Check if the file exists
            if not os.path.exists(save_path_ii):
                raise FileNotFoundError(f"File {save_path_ii} not found, Dataset is not formatted/not found")
            # Append the file to the results
            results.append(save_path_ii)
    ...
    def get_gt_seg_maps(self, efficient_test=False):
        ...
        for video_imgname in self.img_all:
            video, img_name=video_imgname[0], video_imgname[1]
            if self.dilation_mode:
                im_idx = self.imgdic[video].index(img_name)
                if im_idx<self.max_dilation-1:
                    continue
            seg_map = img_name.replace(self.img_suffix, self.seg_map_suffix)
            ann_dir=os.path.join(self.data_root,'data',video,'mask/')
    ...
    def evaluate(self,
        ...
        for i in range(1, len(summary_table_data[0])):
            eval_results[summary_table_data[0]
                         [i]] = summary_table_data[1][i] / 100.0
        if mmcv.is_list_of(results, str):
            if 'keep_results' in kwargs:
                print("Keeping the results as the flag to keep_results is True")
                if not kwargs['keep_results']:
                    for file_name in results:
                        os.remove(file_name)
        return eval_results
