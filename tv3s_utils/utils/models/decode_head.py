...
class BaseDecodeHead(nn.Module, metaclass=ABCMeta):
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        if len(gt_semantic_seg.shape)==5:
            batch_size, num_clips, _, h ,w=gt_semantic_seg.shape
            gt_semantic_seg=gt_semantic_seg.reshape(batch_size*num_clips,-1,h,w)
...
class BaseDecodeHead_clips_flow(nn.Module, metaclass=ABCMeta):
    ...
    def forward_test(self, inputs, img_metas, test_cfg, batch_size=None, num_clips=None, img=None):
        ...
        if 'img_metas' in inspect.getfullargspec(self.forward).args:
    ...
    def consistency_loss(self, preds, gts, batch_size):
        # preds: b*k
