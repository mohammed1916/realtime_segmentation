...
def preprocess_image(img, img_norm_cfg, crop_size=(512, 512)):
    """Manually preprocess image for SegFormer inference"""
    # Convert BGR to RGB
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    # Resize to model input size (keep aspect ratio)
    h, w = img.shape[:2]
    scale = min(crop_size[0] / h, crop_size[1] / w)
...
def main():
    ...
    try:
        while True:
            ...
            # create result dict for visualization
            ...
            # visualize
            draw_img = show_result_pyplot(model, frame, result_sample, opacity=args.opacity, show=False)
